...
def load_trained_model():
    ...
    model.load_weights('x_ray_analysis_model.h5')
    return model
...
if uploaded_file is not None:
    try:
        ...
        if not is_chest_xray(img):
            ...
            results_df = pd.DataFrame({"Condition": labels, "Confidence Score": predictions[0]*100})
            results_df["Confidence Score"] = results_df["Confidence Score"].apply(lambda x: math.floor(x))
            results_df = results_df[results_df["Confidence Score"] > 55]
            if len(results_df) == 0:
                st.subheader("X-Ray Is Normal!")
            else:
            ...
            # styled_df = results_df.style.apply(lambda x: ['background-color: yellow' if v == x.max() else '' for v in x], subset=['Probability'])
                ...
                st.dataframe(results_df, width=700,hide_index=True)
...
st.subheader("If want to upload a new image click on Browse!!!")
